...
Процедура ПрочитатьНаСервере()
	...
	Если Не ПодключитьВнешнююКомпоненту(ФайлКомпоненты, "X", ТипВнешнейКомпоненты.Native, ТипПодключенияВнешнейКомпоненты.НеИзолированно) Тогда
		ВызватьИсключение "Не удалось подключить компоненту";
	КонецЕсли;
	
	Parser = Новый ("Addin.X.TechLogParser");
